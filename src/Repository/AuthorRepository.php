...
use BlackCat\Database\Packages\Authors\Repository\AuthorRepositoryInterface;
...
class AuthorRepository implements AuthorRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'slug_ci' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Authors\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'name', 'bio', 'photo_url', 'story', 'books_count', 'ratings_count', 'rating_sum', 'avg_rating', 'last_rating_at', 'updated_at', 'version', 'deleted_at' ]);
          return count($rows);
        }
