...
namespace BlackCat\Database\Packages\Authors\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Authors\Definitions;
use BlackCat\Database\Packages\Authors\Criteria;
use BlackCat\Database\Packages\Authors\Dto\AuthorDto as Dto;
use BlackCat\Database\Packages\Authors\Mapper\AuthorDtoMapper as RowMapper;
...
final class AuthorRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Authors\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $keys = [ 'tenant_id', 'slug_ci' ];
        if (!$keys) {
            $uqs = Definitions::uniqueKeys();
            $keys = (is_array($uqs) && isset($uqs[0]) && is_array($uqs[0]) && $uqs[0]) ? $uqs[0] : $this->pkColumns(Definitions::class);
        }

        $updCols = [ 'name', 'bio', 'photo_url', 'story', 'books_count', 'ratings_count', 'rating_sum', 'avg_rating', 'last_rating_at', 'updated_at', 'version', 'deleted_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'slug_ci' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Authors\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'slug_ci' ], [ 'name', 'bio', 'photo_url', 'story', 'books_count', 'ratings_count', 'rating_sum', 'avg_rating', 'last_rating_at', 'updated_at', 'version', 'deleted_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null */
    public function getByTenantIdAndSlugCiAndIsLive(int $tenantId, string $slugCi, int $isLive, bool $asDto = false): array|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'slug_ci' => $slugCi, 'is_live' => $isLive ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Authors\Mapper\AuthorDtoMapper::fromRow($row);
    }
    ...
    public function getIdByTenantIdAndSlugCiAndIsLive(int $tenantId, string $slugCi, int $isLive) {
        $row = $this->getByTenantIdAndSlugCiAndIsLive($tenantId, $slugCi, $isLive);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Authors\Dto\AuthorDto|null {
